...
export default class TextSelectionController extends Controller {
  ...
  connect() {
    ...
    this.element.addEventListener('mouseup', this.boundHandleTextSelection)
    document.addEventListener('click', this.boundHandleDocumentClick)
  }

  disconnect() {
    this.element.removeEventListener('mouseup', this.boundHandleTextSelection)
  ...
  handleDocumentClick(event) {
    // Only hide if clicking outside the button and not on the log lines container
    if (!this.buttonTarget.contains(event.target) && !this.element.contains(event.target)) {
      this.hideButton()
    }
  }
